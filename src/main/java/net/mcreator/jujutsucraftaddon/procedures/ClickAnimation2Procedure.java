											...
											while (iterator.hasNext()) {
												...
												if (!connection.isConnecting() && connection.isConnected())
													JujutsucraftaddonMod.PACKET_HANDLER.sendTo(
															new SetupAnimationsProcedure.JujutsucraftaddonModAnimationMessage(Component.literal(("attack" + Mth.nextInt(RandomSource.create(), 2, 39))), entity.getId(), false), connection,
				...
				if (entity instanceof Player && world instanceof ServerLevel srvLvl_) {
					...
					synchronized (connections) {
						...
						while (iterator.hasNext()) {
							Connection connection = iterator.next();
							if (!connection.isConnecting() && connection.isConnected())
								JujutsucraftaddonMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.JujutsucraftaddonModAnimationMessage(Component.literal(("sukuna" + Mth.nextInt(RandomSource.create(), 1, 26))), entity.getId(), false),
										connection, NetworkDirection.PLAY_TO_CLIENT);
						}
					}
				}

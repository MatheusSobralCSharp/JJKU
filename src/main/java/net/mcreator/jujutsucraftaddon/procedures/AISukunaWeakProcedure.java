...
public class AISukunaWeakProcedure {
	public static void execute(LevelAccessor world, Entity sourceentity) {
		if (sourceentity == null)
			return;
		if (sourceentity instanceof SukunaNoShirtEntity) {
			if (!(new Object() {
				...
				if (Math.random() >= 0.8) {
					if (new Object() {
						...
						if (Math.random() >= 0.9) {
							...
							dataIndex10.getCompound("ForgeData").putDouble("skill", (Mth.nextInt(RandomSource.create(), 105, 106)));
							...
							if (Math.random() < 0.4) {
								...
								dataIndex22.getCompound("ForgeData").putDouble("skill", (Mth.nextInt(RandomSource.create(), 105, 106)));

						...
						synchronized (connections) {
							...
							while (iterator.hasNext()) {
								...
								if (!connection.isConnecting() && connection.isConnected())
									JujutsucraftaddonMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.JujutsucraftaddonModAnimationMessage(Component.literal("rapiddismantle"), entity.getId(), false), connection,
					...
					if (entity instanceof Player && world instanceof ServerLevel srvLvl_) {
						...
						synchronized (connections) {
							...
							while (iterator.hasNext()) {
								Connection connection = iterator.next();
								if (!connection.isConnecting() && connection.isConnected())
									JujutsucraftaddonMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.JujutsucraftaddonModAnimationMessage(Component.literal("maximum3"), entity.getId(), false), connection, NetworkDirection.PLAY_TO_CLIENT);
							}
						}
					}
				}
				...
				if (entity instanceof Player && world instanceof ServerLevel srvLvl_) {
					...
					synchronized (connections) {
						...
						while (iterator.hasNext()) {
							Connection connection = iterator.next();
							if (!connection.isConnecting() && connection.isConnected())
								JujutsucraftaddonMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.JujutsucraftaddonModAnimationMessage(Component.literal("spiderwebanim"), entity.getId(), false), connection, NetworkDirection.PLAY_TO_CLIENT);
						}
					}
				}
			}

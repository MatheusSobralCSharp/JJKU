			...
			if (((sourceentity.getCapability(JujutsucraftaddonModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JujutsucraftaddonModVariables.PlayerVariables())).Mode).equals("Dismantle")) {
						...
						if (Math.random() < (1) / ((float) 10)) {
							{
								...
								if (!projectileLevel.isClientSide()) {
									...
									}.getArrow(projectileLevel, sourceentity, 5, 2, (byte) 1);
				...
				if (sourceentity instanceof ServerPlayer && ((ServerPlayer) sourceentity).level() instanceof ServerLevel
					...
					if (Math.random() < (1) / ((float) 50)) {
						...
						sourceentity.getPersistentData().putDouble("Damage", (double) (6 + Mth.nextInt(RandomSource.create(), 1, 2) * 3));
						sourceentity.getPersistentData().putDouble("Range", (double) (entity.getBbWidth() + 10.0F));

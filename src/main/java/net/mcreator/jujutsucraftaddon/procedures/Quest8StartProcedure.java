	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof ServerLevel _serverLevel) {
			...
			if (entitytospawn != null) {
				...
				dataIndex13.getCompound("ForgeData").putString("OWNER_UUID", ((entitytospawn).getStringUUID()));
				...
				dataIndex28.getCompound("ForgeData").putDouble("TenShadowsTechnique14", (-2));
				(entitytospawn).load(dataIndex28);
				CompoundTag dataIndex31 = new CompoundTag();
				(entitytospawn).saveWithoutId(dataIndex31);
				dataIndex31.getCompound("ForgeData").putDouble("friend_num2", (new Object() {
					public double getValue() {
						CompoundTag dataIndex30 = new CompoundTag();
						entity.saveWithoutId(dataIndex30);
						return dataIndex30.getCompound("ForgeData").getDouble("friend_num");
					}
				}.getValue()));
				(entitytospawn).load(dataIndex31);
		...
		if (world instanceof ServerLevel _serverLevel) {
			...
			if (entitytospawn != null) {
				...
				dataIndex47.getCompound("ForgeData").putDouble("brokenBrain", 2);

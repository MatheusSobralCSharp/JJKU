		...
		if (BoolArgumentType.getBool(arguments, "Team")) {
			if ((entity.getCapability(JujutsucraftaddonModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JujutsucraftaddonModVariables.PlayerVariables())).PVP == true && ((new Object() {
				...
				if (world instanceof ServerLevel _serverLevel) {
					Entity entitytospawn = JujutsucraftaddonModEntities.VEIL.get().spawn(_serverLevel,
				...
				if (world instanceof ServerLevel _serverLevel) {
					Entity entitytospawn = JujutsucraftaddonModEntities.VEIL.get().spawn(_serverLevel,
							BlockPos.containing(JujutsucraftaddonModVariables.MapVariables.get(world).X1, JujutsucraftaddonModVariables.MapVariables.get(world).Y1, JujutsucraftaddonModVariables.MapVariables.get(world).Z1), MobSpawnType.MOB_SUMMONED);
					if (entitytospawn != null) {
						entitytospawn.setYRot(world.getRandom().nextFloat() * 360.0F);
					}
					if ((entitytospawn) instanceof VeilEntity _datEntSetI)

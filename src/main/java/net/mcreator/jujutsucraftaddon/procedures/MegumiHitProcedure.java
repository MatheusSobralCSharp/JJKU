    ...
    public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
        ...
        if (Math.random() < (1) / ((float) 50)) {
                ...
                if (world instanceof ServerLevel _serverLevel) {
                    ...
                    if (entitytospawn != null) {
                        ...
                        ((LivingEntity) (entitytospawn)).getAttribute(ForgeRegistries.ATTRIBUTES.getValue(new ResourceLocation("jujutsucraft:size"))).setBaseValue(5);
                        {
                            ...
                            dataIndex.getCompound("ForgeData").putBoolean("Giant", true);
            ...
            if (new Object() {
                ...
                if (world instanceof ServerLevel _serverLevel) {
                    Entity entitytospawn = JujutsucraftModEntities.MAX_ELEPHANT.get().spawn(_serverLevel, BlockPos.containing((entity.getX()), (entity.getY() + 30), (entity.getZ())), MobSpawnType.MOB_SUMMONED);
                    if (entitytospawn != null) {
                        entitytospawn.setYRot(world.getRandom().nextFloat() * 360.0F);
                        CompoundTag dataIndex1 = new CompoundTag();
                        (entitytospawn).saveWithoutId(dataIndex1);
                        ...
                        (entitytospawn).load(dataIndex3);
                        if ((entitytospawn) instanceof LivingEntity _entity && !_entity.level().isClientSide())
                            _entity.addEffect(new MobEffectInstance(JujutsucraftaddonModMobEffects.MANIFESTATION.get(), 80, 1, false, false));
                        {
                            CompoundTag dataIndex = new CompoundTag();
                            (entitytospawn).saveWithoutId(dataIndex);

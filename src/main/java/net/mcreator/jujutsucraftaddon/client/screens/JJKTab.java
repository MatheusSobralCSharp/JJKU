...
public abstract class JJKTab {
    ...
    protected final JujutsuMenuScreen screen;
    ...
    public JJKTab(Minecraft minecraft, JujutsuMenuScreen screen, JJKTabType type, int index, int page, ItemStack icon, Component title, boolean scrollable) {
    ...
    public JujutsuMenuScreen getScreen() {
        return this.screen;
    }
    ...
    public void scroll(double pDragX, double pDragY) {
        ...
        if (this.maxX - this.minX > JujutsuMenuScreen.WINDOW_INSIDE_WIDTH) {
            this.scrollX = Mth.clamp(this.scrollX + pDragX, -(this.maxX - JujutsuMenuScreen.WINDOW_INSIDE_WIDTH), 0.0D);
        }
        if (this.maxY - this.minY > JujutsuMenuScreen.WINDOW_INSIDE_HEIGHT) {
            this.scrollY = Mth.clamp(this.scrollY + pDragY, -(this.maxY - JujutsuMenuScreen.WINDOW_INSIDE_HEIGHT), 0.0D);
        }
    }
    ...
    public void drawContents(GuiGraphics pGuiGraphics, int pX, int pY) {
        ...
        pGuiGraphics.enableScissor(pX, pY, pX + JujutsuMenuScreen.WINDOW_INSIDE_WIDTH, pY + JujutsuMenuScreen.WINDOW_INSIDE_HEIGHT);
        ...
        for (int i1 = -1; i1 <= 15; ++i1) {
            for (int j1 = -1; j1 <= 8; ++j1) {
                pGuiGraphics.blit(BACKGROUND, k + JujutsuMenuScreen.BACKGROUND_TILE_WIDTH * i1, l + JujutsuMenuScreen.BACKGROUND_TILE_HEIGHT * j1, 0.0F, 0.0F,
                        JujutsuMenuScreen.BACKGROUND_TILE_WIDTH, JujutsuMenuScreen.BACKGROUND_TILE_HEIGHT, JujutsuMenuScreen.BACKGROUND_TILE_WIDTH, JujutsuMenuScreen.BACKGROUND_TILE_HEIGHT);
            }
        }

...
import net.mcreator.jujutsucraftaddon.network.AltarMessageGojo;
...
public class GojoScreen extends Screen {
    ...
    protected void init() {
        ...
        this.pages.clear();
        List<GojoScreen.AltarOption> options = this.getOptions();
    ...
    private List<GojoScreen.AltarOption> getOptions() {
        ...
        options.add(new GojoScreen.AltarOption("Barrage Attack"));
        options.add(new GojoScreen.AltarOption("Run Attack"));
        ...
        options.add(new GojoScreen.AltarOption("Unlimited Void(WIP)"));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < this.getCurrent().size(); i++) {
            ...
            GojoScreen.AltarOption item = this.getCurrent().get(i);
            int white = toRGB24(255, 255, 255, 150);
            int black = toRGB24(51, 153, 255, 150);
            ...
            if (this.hovered > 0) {
                ...
                AltarMessageGojo.pressAction(Minecraft.getInstance().player, this.hovered);
    ...
    private List<? extends GojoScreen.AltarOption> getCurrent() {
    ...
    public void onClose() {
        ...
        List<? extends GojoScreen.AltarOption> items = this.pages.get(page);

        if (this.hovered >= items.size()) return;

        GojoScreen.AltarOption item = items.get(this.hovered);
        item.onSelect();
    }

...
import net.mcreator.jujutsucraftaddon.network.AltarMessageSukuna;
...
public class SukunaScreen extends Screen {
    ...
    protected void init() {
        ...
        this.pages.clear();
        List<SukunaScreen.AltarOption> options = this.getOptions();
    ...
    private List<SukunaScreen.AltarOption> getOptions() {
        ...
        options.add(new SukunaScreen.AltarOption("Attack"));
        options.add(new SukunaScreen.AltarOption("Knockback Attack"));
        options.add(new SukunaScreen.AltarOption("Barrage Attack"));
        ...
        options.add(new SukunaScreen.AltarOption("Malevolent Shrine(WIP)"));
        return options;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < this.getCurrent().size(); i++) {
            ...
            SukunaScreen.AltarOption item = this.getCurrent().get(i);
            int white = toRGB24(255, 255, 255, 150);
            int black = toRGB24(200, 25, 25, 150);
            ...
            if (this.hovered > 0) {
                ...
                AltarMessageSukuna.pressAction(Minecraft.getInstance().player, this.hovered);
    ...
    private List<? extends SukunaScreen.AltarOption> getCurrent() {
    ...
    public void onClose() {
        ...
        List<? extends SukunaScreen.AltarOption> items = this.pages.get(page);

        if (this.hovered >= items.size()) return;

        SukunaScreen.AltarOption item = items.get(this.hovered);
        item.onSelect();
    }
